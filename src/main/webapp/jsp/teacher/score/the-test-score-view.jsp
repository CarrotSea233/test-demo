<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"
    ...
    <%
        ...
        Test theTest = testMapper.getTestById(testId);

        TestStudentView test = new TestStudentView(testId);
        List<TestStudentView> tests = testMapper.findTestStudentViews(test);

        Integer allScore = 0;
        Integer maxScore = 0;
        Integer minScore = 1000000;
        for(int i = 0; i < tests.size(); i++) {
            Integer theScore = Integer.parseInt(tests.get(i).getTotalScore());
            allScore += theScore;
            if(theScore > maxScore) maxScore = theScore;
            if(theScore < minScore) minScore = theScore;
        }
        Integer avgScore = 0;
        if(tests.size() != 0) avgScore = allScore / tests.size();
    ...
    <head>
        ...
        <title><%=theTest.getTestName()%></title>
            ...
            <div class="layui-side layui-bg-black">
                <div class="layui-side-scroll">
                    <ul class="layui-nav layui-nav-tree">
                        ...
                        <li style="background-color: #16baaa;" class="layui-nav-item"><a style="color: white;" href="/test-demo/jsp/teacher/score/score-view.jsp">成绩管理</a></li>
                    ...
                    <div class="layui-panel" style="min-height: calc(100vh - 60px - 32px);">
                        <div style="padding: 15px;">
                            ...
                            <table class="layui-table">
                                <tr>
                                    <td style="border: none;">平均分：<%=avgScore%></td>
                                    <td style="border: none;">最高分：<%=maxScore%></td>
                                    <td style="border: none;">最低分：<%=minScore%></td>
                                </tr>
                            </table>
                                    ...
                                    <% 
                                        for(int i=0; i<tests.size(); i++) {
                                            out.print("<tr>"+
                                                ...
                                                "<td><button type='button' class='layui-btn' onclick='toTheStudentTestScore("+ tests.get(i).getTestId()+","+tests.get(i).getUserId()+")'>查看</button></td>"+
                                                "</tr>");
                                        } 
                                    %>
                                </tbody>
